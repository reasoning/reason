...
void Formatter::FormatVa(String & string, const char * format, va_list va)
{
	...
	while (true) 
	{
		...
		va_copy(val,va);
		count = vsnprintf (string.Data, allocated, format, val);
		va_end(val);
...
int FormattingStream::Read(Value & value)
{
	...
	{	
		...
		case 16:
			if (parser.IsHex())
			{
				...
				value.Number = Sequences::Hex(parser.Token.Data,parser.Token.Size);
				parser.Trap(token);
				value.String = token;
		...
		case 10:
			if (parser.IsDecimal())
			{
				...
				value.Number = Sequences::Decimal(parser.Token.Data,parser.Token.Size);
				parser.Trap(token);
				value.String = token;
				value = Value::TYPE_NUMBER;
			}
		...
		case 8:
			if (parser.IsRange('0','7'))
			{
				...
				value.Number = Sequences::Octal(parser.Token.Data,parser.Token.Size);
				parser.Trap(token);
				value.String = token;
				value = Value::TYPE_NUMBER;
			}
	...
	if (specifier.Is(Specifier::TYPE_FORMAT_e|Specifier::TYPE_FORMAT_E|Specifier::TYPE_FORMAT_f|Specifier::TYPE_FORMAT_g|Specifier::TYPE_FORMAT_G))
	{
		...
		if (numeric)
		{
			value.Number = Sequences::Real(parser.Token.Data,parser.Token.Size);
			parser.Trap(token);
			value.String = token;
			value = Value::TYPE_NUMBER;
		}
