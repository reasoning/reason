...
class RegexIdentifier: public Bitmask32
{
public:
	...
	static Bitmask32 Classify(char data)
	{
		Bitmask32 token;
		...
		switch (data)
		{
		case Character::Backspace: token.On(IDENTIFIER_BACKSPACE); break;
		case Character::Tab: token.On(IDENTIFIER_TAB); break;
		case Character::CarriageReturn: token.On(IDENTIFIER_CARRIAGE_RETURN); break;
		case Character::VerticalTab: token.On(IDENTIFIER_VERTICAL_TAB); break;
		case Character::FormFeed: token.On(IDENTIFIER_FORM_FEED); break;
		case Character::NewLine: token.On(IDENTIFIER_NEW_LINE); break;
		}						
		...
		if (Character::IsAlphanumeric(data) || data == '_')
		{
			...
			token.On(Character::IsNumeric(data)?IDENTIFIER_DIGIT:IDENTIFIER_NON_DIGIT);
		}
		else
		{
			token.On(IDENTIFIER_NON_WORD);
			token.On(IDENTIFIER_NON_DIGIT);
			token.On(Character::IsWhitespace(data)?IDENTIFIER_WHITESPACE:IDENTIFIER_NON_WHITESPACE);
		}
...
public:
	...
	static Bitmask8 Classify(char data)
	{
		Bitmask8 range;
...
public:
	...
	RegexState * Parent;
	Reason::Structure::Array<RegexState*> Children;
...
public:
	...
	RegexFrame(RegexState * state, int frame=0):State(state),Frame(frame) {}
...
class RegexScope : public Reason::Structure::Map<RegexState*,RegexState*>
{
public:
	...
	using Reason::Structure::Map<RegexState*,RegexState*>::operator [];
};

template <typename _Kind_>
class RegexArray : public Reason::Structure::Array<_Kind_>
{
public:
	...
	void Push(RegexArray & stack)
	{
		...
		if (amount > 0)
		{
			...
			OutputAssert(this->Allocated-this->Size > amount);

			for (int a = 0;a < amount;++a)
				this->Data[this->Size+a] = stack.Data[a];
			this->Size += amount;

			OutputAssert(this->Size <= this->Allocated);
		}
...
public:
	...
	RegexState * State;
	Reason::Structure::Array< Reason::Structure::Array<Substring> > * Matches;
	int Options;

	RegexInterpreter(RegexState * state, Reason::Structure::Array< Reason::Structure::Array<Substring> > * matches):
	...
	int Match(Sequence & sequence) {return Match(1,sequence.Data,sequence.Size);}
	...
	int Match(int limit, Sequence & sequence) {return Match(limit,sequence.Data,sequence.Size);}
	...
	int Match(int limit, char * data, int size)
	{
		...
		while (index <= input.Size && (matches < limit || limit == 0))
		{
			...
			#ifdef REASON_REGEX_RECURSION

			if (ProcessBacktrack(input,state,next,capture,base,offset) > -1)
			#else

			if ((offset = ProcessBacktrack((Sequence&)input,state,base)) > -1)
			#endif
			{
				#ifdef REASON_REGEX_RECURSION
	...
	int Process(Sequence & input, RegexState * state, RegexStack<RegexState*> & next, RegexStack<Substring> & capture, int & base, int & offset)
	{
	...
	int ProcessBacktrack(Sequence & input, RegexState * state, RegexStack<RegexState*> & next, RegexStack<Substring> & capture, int & base, int & offset)
	{
		if (state == 0 || base > input.Size)
			return -1;
	...
	bool ProcessTerminal(Sequence & input, RegexFunction * function, int & index, RegexStack<Substring> & capture)
	{
				...
				case RegexFunctionAnchor::ANCHOR_START:
					{
						...
						if ((Options&RegexConstants::OPTION_MULTILINE) && index > 0 && Character::IsNewline(input.Data[index-1]) && !Character::IsNewline(input.Data[index]))
							return true;
					}
					break;
				case RegexFunctionAnchor::ANCHOR_END:
					{
						...
						if ((Options&RegexConstants::OPTION_MULTILINE) && index < input.Size && Character::IsNewline(input.Data[index]) && !Character::IsNewline(input.Data[index-1]))
							return true;
					}
					break;
				...
				case RegexFunctionAnchor::ANCHOR_NON_WORD:
					{
						...
						bool match = false;
						if ((index == 0 && Character::IsIdentifier(input.Data[index])) || (index == input.Size && Character::IsIdentifier(input.Data[index-1])) || 
							((index > 0 && index < input.Size) && 
								(Character::IsIdentifier(input.Data[index]) && !Character::IsIdentifier(input.Data[index-1])) ||
								(!Character::IsIdentifier(input.Data[index]) && Character::IsIdentifier(input.Data[index-1]))))
							match = true;
	...
	int ProcessStack(Sequence & input, RegexState * state, int index = 0)
	{
		Reason::Structure::Map<RegexState*,RegexState*> scope;
	...
	RegexState * ParseTerminal()
	{
		...
		if (Is('\\'))
		{
			if (IsDecimal(1))
			{
				...
				if (Sequences::IsDecimal(Token.Data,Token.Size))
				{
...
public:
	...
	explicit Pattern(const Sequence & sequence):String(sequence),Options(0)
	{
	}
...
public:
	...
	Regex(const Sequence & sequence):State(0) {Construct(sequence);}
	...
	Regex & Construct(const Sequence & sequence) {return Construct(sequence.Data,sequence.Size);}
	...
	Regex & operator = (const Reason::System::Superstring & superstring) {Construct(superstring);return *this;}
	Regex & operator = (const String & data) {Construct(data.Data,data.Size);return *this;}
	Regex & operator = (const Sequence & data) {Construct(data.Data,data.Size);return *this;}
